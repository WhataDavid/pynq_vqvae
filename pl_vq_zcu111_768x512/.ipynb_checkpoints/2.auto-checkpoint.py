...
BIT_PATH = os.path.join(WORK_DIR, 'pl_vq_zcu111_768x512/dpu.bit')

print("⏳ 连接已加载的 overlay（不重新下载 bitstream）...", flush=True)
print("⚠️ 请确保你已经先运行过 1.load_bit.py", flush=True)

overlay = DpuOverlay(BIT_PATH, download=False)
vq_ip = overlay.vq_accel_1
print("✅ overlay connected:", vq_ip, flush=True)

enc_out_scale = 0.015625
dec_in_scale  = 0.03125
dec_out_scale = 0.007812

# 768x512 -> latent 128x192
num_vectors = 128 * 192
dim = 64
...
enc_graph, enc_subgraph = get_dpu_subgraph(os.path.join(WORK_DIR, 'xmodel/encoder_768x512.xmodel'))
enc_runner = vart.Runner.create_runner(enc_subgraph, "run")
dec_graph, dec_subgraph = get_dpu_subgraph(os.path.join(WORK_DIR, 'xmodel/decoder_768x512.xmodel'))
...
def enc_worker():
    while True:
        ...
        # encoder 输出直接写入 VQ 输入 buffer
        out_buf = [np.ndarray((1, 128, 192, 64), dtype=np.int8, buffer=target_vq_buf.data)]
...
def phase1_pipeline():
    print(f"🚀 [Phase 1] Starting Read -> Encoder -> VQ Pipeline: {num_imgs} images", flush=True)
...
def dec_worker():
    for zq_res in all_zq_results:
        z_q_int8 = zq_res.reshape(1, 128, 192, 64)
        dec_in_buf = [z_q_int8]
...
def phase2_pipeline():
    print(f"🚀 [Phase 2] Starting Decoder -> LUT Pipeline: {num_imgs} images", flush=True)
...
if __name__ == "__main__":
    ...
    print("🖼️ 正在写入最终图片...", flush=True)
    for i, img in enumerate(all_recon_imgs):
        cv2.imwrite(f'{RES_DIR}/recon_{i}.png', cv2.cvtColor(img, cv2.COLOR_RGB2BGR))

    print(f"✅ Pipeline perfectly completed. Saved {len(all_recon_imgs)} images.", flush=True)